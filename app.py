...
def parse_model_id(user_input: str) -> str:
    ...
    # URL form: https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct
    ...
    if "/" in user_input:
        return user_input
    # bare model name – return as-is so HF search can try
    return user_input
...
def estimate_params_from_config(config: dict) -> int | None:
    ...
    try:
        ...
        if h and n_layers and v:
            if i is None:
                i = 4 * h  # common default
            # Approximate: embed + n_layers*(attn + ffn) + lm_head
            params = v * h  # embedding
            params += n_layers * (4 * h * h)  # attention (Q, K, V, O)
            params += n_layers * (2 * h * i)  # FFN up + down
            params += v * h  # lm_head (sometimes tied)
            return int(params)
    except Exception:
        pass
    return None
...
def extract_param_count(model_info: dict, config: dict | None) -> float | None:
    ...
    # 2. model card metadata  (often has "parameter_count" or similar)
    ...
    # 3. model name heuristic  (e.g. "Llama-3.1-70B")
...
def estimate_arch_from_params(param_billions: float) -> dict:
    ...
    if params < 1e9:
        return {"num_hidden_layers": 12, "hidden_size": 768, "num_kv_heads": 12, "num_attention_heads": 12}
    elif params < 3e9:
        return {"num_hidden_layers": 24, "hidden_size": 2048, "num_kv_heads": 16, "num_attention_heads": 16}
    elif params < 10e9:
        return {"num_hidden_layers": 32, "hidden_size": 4096, "num_kv_heads": 8, "num_attention_heads": 32}
    elif params < 20e9:
        return {"num_hidden_layers": 40, "hidden_size": 5120, "num_kv_heads": 8, "num_attention_heads": 40}
    elif params < 40e9:
        return {"num_hidden_layers": 48, "hidden_size": 6144, "num_kv_heads": 8, "num_attention_heads": 48}
    elif params < 80e9:
        return {"num_hidden_layers": 64, "hidden_size": 8192, "num_kv_heads": 8, "num_attention_heads": 64}
    else:
        return {"num_hidden_layers": 80, "hidden_size": 8192, "num_kv_heads": 8, "num_attention_heads": 64}
...
def calculate_theoretical_tpm(vm: dict, model_size_gb: float,
    ...
    For MoE models, `active_param_billions` is the number of params
    ...
    available_vram = vm["vram_gb"] - model_size_gb - 3
...
def get_llm_analysis(model_id: str, model_info: dict, config: dict | None,
    ...
    """Use Claude to provide a rich, contextual analysis of the model's memory footprint."""
    api_key = os.getenv("ANTHROPIC_API_KEY")
    model_name = os.getenv("LLM_MODEL", "claude-sonnet-4-20250514")

    if not api_key:
        return "⚠️ No API key configured. Set ANTHROPIC_API_KEY in your .env file to enable LLM analysis."
